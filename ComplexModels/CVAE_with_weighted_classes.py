...
class ConvConditionalVAE(nn.Module):
    def __init__(self, latent_size, num_classes):
        ...
        self.class_scale = nn.Linear(num_classes, latent_size)
    ...
    def forward(self, x, labels):
        ...
        class_probs = F.log_softmax(class_logits, dim=1)

        #Скалируем вероятности классов
        class_scaled = self.class_scale(class_probs)
        # Умножаем коды классов на вероятности классов
        weighted_z = z * class_scaled

        # Склеиваем коды классов с кодами изображений
        concat_z = torch.cat([weighted_z, labels], dim=1)

        x_recon = self.decode(concat_z)

        return x_recon, mu, logvar, class_probs
...
def train_conditional_vae(model, dataloader, num_epochs, learning_rate):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (images, labels) in enumerate(dataloader):
            ...
            # Прямой проход
            recon_images, mu, logvar, class_logits = model(images, one_hot_labels)

            # Вычисление функции потерь
            loss = vae_loss(recon_images, images, mu, logvar, class_logits, labels)
...
def vae_loss(recon_images, images, mu, logvar, class_logits, labels):
    # Reconstruction loss
